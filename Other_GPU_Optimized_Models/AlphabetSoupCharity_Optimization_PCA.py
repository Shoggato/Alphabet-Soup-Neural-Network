...
#Reduce the dimensionality to include 95% of the data
...
def create_model():
    ...
    tf.keras.layers.Dense(97, kernel_regularizer=regularizers.l1(0.03), activation='relu'),
    tf.keras.layers.Dropout(0.07),
    tf.keras.layers.Dense(129, activation='relu'),
    tf.keras.layers.Dense(417, kernel_regularizer=regularizers.l1(0.03), activation='relu'),
...
#train the model
fit_model_nn = nn_model.fit(X_reduced_train, y_train, epochs=300, callbacks=[stop_early])

# Evaluate the model using the test data
model_loss, model_accuracy = nn_model.evaluate(X_reduced_test,y_test,verbose=2)
