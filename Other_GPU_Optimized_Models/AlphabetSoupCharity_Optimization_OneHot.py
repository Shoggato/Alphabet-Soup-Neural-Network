...
def create_model(hp):
    ...
    for i in range(hp.Int('num_layers', 1, 5)):
        model.add(tf.keras.layers.Dense(units=hp.Int('units_' + str(i),
            ...
            step=32),
            activation=activation,
            kernel_regularizer=tf.keras.regularizers.l1(l=hp.Float('l1_weight', min_value=0.00001, max_value=0.1))
            ))
    
    #this is my drop out layer
    model.add(tf.keras.layers.Dropout(hp.Float('dropout', 0.0, 0.9)))
